...
class ContactDetector(contactListener):
    ...
    def BeginContact(self, contact):
        ...
        # lander 본체가 장애물에 닿으면 충돌
        for obstacle in self.env.obstacles:
            if self.env.lander in bodies and obstacle in bodies:
                self.env.obstacle_contact = True
                self.env.game_over = True

            # 다리가 장애물에 닿아도 충돌로 볼지 선택
            for i in range(2):
                if self.env.legs[i] in bodies and obstacle in bodies:
                    self.env.obstacle_contact = True
                    self.env.game_over = True

        # lander 본체가 지면에 닿으면 추락
        if self.env.lander in bodies:
            self.env.game_over = True
...
class LunarLander(gym.Env, EzPickle):
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def _destroy(self):
        ...
        # obstacle 제거해서 reset할 때 이전 장애물 남지 않게
        ...
        *,
        ...
        for ox, oy in obstacle_positions:
            obstacle = self.world.CreateStaticBody(
                position=(ox, oy),
                fixtures=fixtureDef(
                    shape=polygonShape(box=(obstacle_w / 2, obstacle_h / 2)),
                    density=0.0,
                    friction=0.5,
                    categoryBits=0x0001,
                    maskBits=0xFFFF,
            ...
            obstacle.color1 = (220, 50, 50)
            obstacle.color2 = (120, 20, 20)
        ...
        self.drawlist = self.obstacles + [self.lander] + self.legs
    ...
    def step(self, action):
        ...
        for obstacle in self.obstacles:
            rel_x = (obstacle.position.x - pos.x) / (VIEWPORT_W / SCALE / 2)
            rel_y = (obstacle.position.y - pos.y) / (VIEWPORT_H / SCALE / 2)
            obstacle_features.extend([rel_x, rel_y])
        ...
        elif self.game_over or abs(state[0]) >= 1.0:
            terminated = True
            reward = -100
...
if __name__ == "__main__":
    ...
    for episode in range(5):
        ...
        print(f"\nEpisode {episode + 1}")
        print("Initial obs shape:", observation.shape)
        print("Initial obs:", observation)
        print("Obstacle relative features:", observation[8:])
        ...
        for step in range(1000):
            ...
            # 50 step마다 상태 출력
            if step % 50 == 0:
                print(f"step={step:4d} | reward={reward:8.2f} | obs_shape={observation.shape} | obstacle_contact={env.obstacle_contact}")

            if terminated or truncated:
                print(f"Episode {episode + 1} | steps={step + 1} | total_reward={total_reward:.2f} | obstacle_contact={env.obstacle_contact} | game_over={env.game_over}")
